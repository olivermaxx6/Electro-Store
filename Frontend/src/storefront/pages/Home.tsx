...
// @ts-ignore
import { productRepo, categoryRepo, brandRepo } from '../lib/repo.js';
...
const Home: React.FC = () => {
  ...
  // Filter for parent categories - simplified approach
  const parentCategories = categories.filter(cat => {
    ...
    // Simple check: category is a parent if it has no parent
    const isParent = !cat.parent && !cat.parent_id && !cat.parentId;
    
    if (isParent) {
      console.log(`✅ Found parent category: ${cat.name}`);
    }
  ...
  console.log(`📊 Categories Summary:`, {
    ...
    parentCategoryNames: parentCategories.map(cat => cat.name)
  });
  
  // Additional debugging for the homepage display
  console.log(`🏠 Homepage Display Debug:`, {
    showingCategories: parentCategories.length > 0,
    willShowFallback: parentCategories.length === 0,
    categoriesFromRedux: categories.length,
    firstCategory: categories[0] || 'No categories'
  });
  
  // Categories are now fetched through the repo in the main useEffect
